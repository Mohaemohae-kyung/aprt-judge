...
## OpenRouter Reward LLM 연동

`APIAgentClient`는 OpenRouter chat completions API를 사용한다. 하나의
`OPENROUTER_API_KEY`를 사용하고, Rs/Rh는 모델명으로 분리한다.
...
실제 key는 repository에 저장하지 않는다. Placeholder 예시는 `.env.example`을 참고한다.
